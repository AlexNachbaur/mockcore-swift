...
///
/// The MockCore platform treats diagnostics as a product feature: every error names where the
/// problem is (file, line and column when available, or a document path) and, when a near-miss
/// exists, suggests the likely fix.
public struct MockError: Error, Hashable, Sendable, CustomStringConvertible {
    /// The broad category of problem, useful for programmatic matching in tests.
    public enum Category: String, Hashable, Sendable {
        /// A malformed input document (a schema, spec, or operation).
        case syntax
        /// A structurally valid but semantically invalid schema or spec.
        case schema
