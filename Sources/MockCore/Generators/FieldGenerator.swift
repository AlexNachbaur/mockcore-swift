...
public struct FieldGenerator: Sendable {
    ...
    /// Produces a value for the field described by `context`, drawing all randomness from
    /// `context.random` so the result is deterministic.
    public let generate: @Sendable (inout GeneratorContext) -> MockValue

    /// Creates a generator from a closure. Prefer the presets where one fits.
    public static func custom(
        scalarTypeName: String = "String",
        _ generate: @escaping @Sendable (inout GeneratorContext) -> MockValue
    ) -> FieldGenerator {
        FieldGenerator(scalarTypeName: scalarTypeName, generate: generate)
    }

    /// Always produces the given value.
    public static func constant(_ value: MockValue) -> FieldGenerator {
        let typeName: String
        switch value {
    ...
    public static func oneOf(_ values: [MockValue]) -> FieldGenerator {
