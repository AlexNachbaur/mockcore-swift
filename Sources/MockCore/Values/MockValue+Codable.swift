...
extension MockValue: Codable {
    public init(from decoder: any Decoder) throws {
        ...
        if container.decodeNil() {
            ...
            self = .string(string)
        } else if let list = try? container.decode([MockValue].self) {
            self = .list(list)
        } else if let object = try? container.decode([String: MockValue].self) {
            self = .object(object)
        } else {
            throw DecodingError.dataCorruptedError(
                in: container,
                debugDescription: "Value is not representable as a mock value"
            )
        }
    }
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        case .enumValue(let value):
            // Enum values serialize as strings on the wire.
            try container.encode(value)
...
extension MockValue {
    ...
    /// Decodes a `MockValue` tree from a JSON string.
    public static func fromJSONString(_ string: String) throws -> MockValue {
