...
@Suite struct FieldGeneratorTests {
    private func generate(_ generator: FieldGenerator, seed: UInt64 = 7) -> MockValue {
    ...
    @Test func oneOfPicksFromTheGivenValues() {
        let options: [MockValue] = [.string("a"), .string("b")]
        for seed in 0..<20 {
...
@Suite struct GeneratorRegistryTests {
    ...
    private func registryValue(field: String, scalarTypeName: String) -> MockValue {
    ...
    @Test func bindingKeysAreSorted() {
        let registry = GeneratorRegistry(bindings: ["User.name": .fullName, "Post.title": .fullName])
        #expect(registry.bindingKeys == ["Post.title", "User.name"])
    }

    @Test func bindAddsAndReplacesABinding() {
        var registry = GeneratorRegistry()
        registry.bind(typeName: "User", fieldName: "email", generator: .constant(.string("pinned@example.com")))
        let value = registry.value(typeName: "User", recordID: "u1", field: "email", scalarTypeName: "String")
        #expect(value == .string("pinned@example.com"))
    }
}
